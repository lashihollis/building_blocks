...
def fetch_patients(base_url="https://hapi.fhir.org/baseR4", max_patients=20):
...
def fetch_observations_for_patient(patient_id, base_url="https://hapi.fhir.org/baseR4"):
    """Fetch BP observations for a specific patient"""
    observations = []
    url = f"{base_url}/Observation"
    
    # Filter for blood pressure (LOINC codes)
    params = {
        "subject": f"Patient/{patient_id}",
        "code": "http://loinc.org|85354-9,http://loinc.org|8480-6,http://loinc.org|8462-4",
        "_count": 10
    }
    ...
    for entry in data.get("entry", []):
        ...
        observations.append({
            ...
            "unit": value.get("unit"),
            "date": obs.get("effectiveDateTime", "")[:10],  # Just date part
...
def fetch_all_data(max_patients=100):
    ...
    for i, patient in enumerate(patients):
        print(f"Fetching observations for patient {i+1}/{len(patients)}: {patient['id']}")
        obs = fetch_observations_for_patient(patient['id'])
